...
当前用户要求优先于本技能。以下规则只用于编辑，不能用来证明文字由 AI 生成。
...
判断办法：改写后的每一项事实、否定、比较、限定和因果关系，都要能回到原文。换词不要求字面重复，但不得改变原意；例句与此边界冲突时，保留原文。
...
改法：可以从正面给判断，但必须保留原文有意义的否定或比较。仅改掉无信息的修辞外壳；若 A 与 B 的区分就是实质观点，保留或等义改写，不删掉 A。真实的自我修正和有依据的对照不因句式命中就强制删除。
...
改法：改成包含全部原项目的并列句或连接词，不改变项目范围。不能为了减少顿号省去项目、添加顺序或扩大概括；无法等义改写时保留。不得改动原有列表结构。
...
>
> ✅ 这个功能上线后用户反馈很好，团队开发效率也明显提升了。下一步优化什么，已经定了。
...
只有命中本条相邻句骨架重复且能保持全部信息时才处理。实测 AI 的案例句密度是人类的 3-4 倍，但“恰好凑三个”是错觉。在已含案例的段落里，三案例占比 AI 0.6%、人类 0.5%，没有差别。所以只在案例确实平级、看得出在凑数时才压缩。叙事推进（先发生什么、怎么做的、结果如何）不算，不要动。
...
改法：只使用原文已有的动作改写；原文没有的人物、能力或事实不能补入。不能把比喻中的夸张改成字面能力承诺。
...
>
> 此句保留，或请作者核实实际能力；不能把“永不疲倦”的比喻改成技术保证。

不改的情况：

- 比喻本身不是 AI 痕迹。实测人类用比喻是 AI 的 2.4 倍，比喻独立成段更是 8 倍。只处理“理想化职业人格”这一种喻体
- 具体的人当喻体（“像一个老师傅”“像一个人脉很广的医生朋友”）。人类就这么写
- 喻体带来了正文没给的理解时。判断办法：整段删掉比喻，如果读者会漏掉某个信息，就保留；即使上一段已经把机制讲完，只要没有命中本条触发标记，仍保持原文
- 小说、散文等文学体裁按原有风格判断
...
> ❌ 本月付费用户从八十名增至一百名。这意味着增加了二十名。
>
> ✅ 本月付费用户从八十名增至一百名，增加了二十名。
...
下列检查只针对明确命中且能等义改写的部分。为保留事实、作者风格或本 Skill 列出的例外而保留原句，不算检查失败；不能为了清零特征牺牲内容。
...
- 改写后每项事实和关系能否回到原文。姓名、数字、日期、引语、来源、否定、比较、限定和因果，任何新增或丢失都必须撤销

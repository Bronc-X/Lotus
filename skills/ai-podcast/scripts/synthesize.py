"""External voice profiles; CPU, MPS or CUDA with resumable paragraph checkpoints."""
import argparse
import hashlib
import json
import os
...
import re
import sys
from synthesis_contract import finite_number, validate_runtime_numbers, validate_output_paths, output_lock
import time
from runtime_support import load_runtime, configure, check_revision
...
def prompt_profiles(voice):
    ...
    for name,profile in prompts.items():
        if not isinstance(name,str) or not re.fullmatch(r'[a-z][a-z0-9_-]*',name):
            raise ValueError(f'Invalid voice reference name: {name!r}')
...
def main():
    ...
    p.add_argument('--voice-profile',type=Path,required=True)
    ...
    voice=read(a.voice_profile)
    voice_root=a.voice_profile.resolve().parent
    ...
    for name,expected in voice['assets'].items():
        asset=(voice_root/'assets'/name).resolve()
        if not asset.is_relative_to((voice_root/'assets').resolve()):raise ValueError('Asset path escapes voice directory.')
        if digest(asset)!=expected:raise ValueError(f'Voice asset changed: {name}')
    ...
    check_revision(cfg,voice)
    prompts={name:VoiceClonePrompt.load(str(voice_root/'assets'/profile['asset'])) for name,profile in profiles.items()}
    info={'voice_id':voice['voice_id'],'torch':torch.__version__,'device':cfg['device'],'dtype':cfg['dtype'],
        'model_revision':voice['model_revision'],'voice_sha256':voice['assets'][profiles[default_reference]['asset']],
    ...
    protected=[a.runtime,a.voice_profile]+[(voice_root/'assets')/n for n in voice['assets']]
    if a.jobs_file:
